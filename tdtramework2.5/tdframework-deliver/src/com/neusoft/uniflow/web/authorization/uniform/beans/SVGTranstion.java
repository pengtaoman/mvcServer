...
public class SVGTranstion implements Serializable{
	...
	private static final long serialVersionUID = 8836037189833710853L;
	...
	//当图像存在负坐标时，用于图像平移
	private int trX = -10;//图像偏移值
	private int trY = -10;//图像偏移值
	...
	public SVGTranstion(NWSession session,Vector activities,String processid)throws Exception{
		...
		this.caclMaxXY(session,activities,processid);
	...
	private void caclMaxXY(NWSession session,Vector activities,String processid) throws Exception{
		String procVersionName = null;
		for (int i=0;i<activities.size();i++){
			NWActDef actdef = (NWActDef)activities.elementAt(i);
			if(procVersionName == null)
			{
				procVersionName = actdef.getProcVersionName();
			}
		...
		Vector transitions = session.getProcDef("", processid, procVersionName,0).openTransitionList();	
