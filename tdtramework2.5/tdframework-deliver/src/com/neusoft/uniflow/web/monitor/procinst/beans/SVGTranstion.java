...
public class SVGTranstion implements Serializable{
	...
	public SVGTranstion(NWProcInst procinst)throws Exception{
		this.pid = procinst.getProcDefID();
		this.initHashtable(procinst);
		this.caclMaxXY(procinst);
		
	}

	//以Hashtable存放 节点列表，记录 x,y,l（长度）
	private void initHashtable(NWProcInst procinst) throws NWException{
		...
		Vector procapps = process.openApplicationList();//流程级应用程序
		...
		Vector applications = session.openApplicationList();//全局应用程序
		for (int i = 0; i < activities.size(); i++) {
			NWActDef actdef = (NWActDef) activities.elementAt(i);
		...
		//对于动态分支实例，第一次展现时，按照时间，
		//留下最后一次产生的动态分支实例以及其他字节点实例，其他的分支以及孩子节点全部移除，后续渲染不与考虑
		Vector actinsts = procinst.openActInstList(-1);
		List removeActInsts = new ArrayList();
		Map multiBranchMap = new HashMap();
		for (int i=0; i<actinsts.size(); i++) {
			NWActInst actinst = (NWActInst) actinsts.elementAt(i);
			if (actinst.isMultiBranch()) {
				NWActInst branchAct=(NWActInst)multiBranchMap.get(actinst.getActDefID());
				if (branchAct!= null) {
					if(actinst.getStartTime().after(branchAct.getStartTime()))
					{
							removeActInsts.addAll(getAllChildInst(branchAct, actinsts));
							multiBranchMap.put(actinst.getActDefID(), actinst);
					}
					else
						removeActInsts.addAll(getAllChildInst(actinst, actinsts));
				} else {
					multiBranchMap.put(actinst.getActDefID(), actinst);
				}
			}
		}
		actinsts.removeAll(removeActInsts);//将过滤掉的动态分支实例移除
		for (int i = 0; i < actinsts.size(); i++) {
			NWActInst actinst = (NWActInst) actinsts.elementAt(i);
			String keyid = actinst.getActDefID();
			if (states.get(keyid)!=null){
				NWActInst ai = getActInst(actinsts,(String)actInstIDs.get(keyid));
				if (actinst.getStartTime().after(ai.getStartTime())){
					putActInstInfo(actinst);
				}
			}else{
				putActInstInfo(actinst);
			}
	...
	private NWActInst getActInst(Vector actList,String actInstID){
		for(int i=0;i<actList.size();i++){
			NWActInst actInst=(NWActInst)actList.get(i);
			if(actInst.getActInstID().equals(actInstID))
				return actInst;
		}
		return null;
	}
	
	private List getAllChildInst(NWActInst parentActInst ,Vector actInsts) throws NWException {
		List childInsts = new ArrayList();
		for (int i=0; i<actInsts.size(); i++)  {
			NWActInst actInst = (NWActInst) actInsts.elementAt(i);
			if (actInst.getParentActInstID().equals(parentActInst.getActInstID())) {
				childInsts.add(actInst);
				if (actInst.getType() == NWActDef.ACT_TYPE_PARALLEL_UNIT || 
						(actInst.getType() == NWActDef.ACT_TYPE_PARALLEL_BRANCH && !actInst.isMultiBranch())) {
					childInsts.addAll(getAllChildInst(actInst, actInsts));
				} 
			}
		}
		return childInsts;
	...
	private void putActInstInfo(NWActInst actinst) throws NWException {
		String keyid = actinst.getActDefID();
		states.put(keyid, String.valueOf(actinst.getCurState()));
		actInstIDs.put(keyid, actinst.getActInstID());
		if(actinst.getTimerState()!= NWActInst.TIME_STATE_NORMAL)
			overtimeInfo.put(keyid, Integer.toString(actinst.getTimerState()));
		if (actinst.getType()==NWActDef.ACT_TYPE_ASYNCHSUBPROC||actinst.getType()==NWActDef.ACT_TYPE_SYNCHSUBPROC) {
			NWProcInst subproc = actinst.getSubProcInst();
			subProc.put(keyid, subproc.getProcInstID());
		}
	}
	 ...
	 * @throws Exception
	 */
	private void caclMaxXY(NWProcInst procinst) throws Exception{
		
		Vector activities = procinst.getProcDef().openActivityList();
		for (int i=0;i<activities.size();i++){
			NWActDef actdef = (NWActDef)activities.elementAt(i);
		...
		Vector transitions = procinst.getProcDef().openTransitionList();	
