...
public class SetReadActPartiAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Vector ptcptEntity = this.getReadParticipant(nwactdef.openParticipantList());
	...
	public String[] getproCreator(Vector ptcptentity) {
		String procreator[] = new String[4];
		String returnprocreator[] = new String[4];
		int ptcptsize = ptcptentity.size();
		int count = 0;
		for (int i = 0; i < ptcptsize; i++) {
			NWParticipantEntity participantentity = (NWParticipantEntity) ptcptentity
					.elementAt(i);
	...
	public Vector openVarActlist(NWActDef actdef, NWSession nwsession,
		...
		NWProcDef nwprocdef = nwsession.getProcDef(userid, processid, version,0);
		Vector varlist = nwprocdef.openRelDataList();
	...
	public String[] getSelectVarlist(Vector partiEntitylist) {
		Vector varparti = new Vector();
		int partisize = partiEntitylist.size();
		for (int i = 0; i < partisize; i++) {
			NWParticipantEntity nwparticipantentity = (NWParticipantEntity) partiEntitylist
					.elementAt(i);
	...
	/**
	 * get the read only participant through the partiEntitylist
	 * */
	public Vector getReadParticipant(Vector partiEntitylist){
		Vector participant=new Vector();
		for(int i=0;i<partiEntitylist.size();i++){
			NWParticipantEntity nwparticipantentity=(NWParticipantEntity)partiEntitylist.elementAt(i);
			if(nwparticipantentity.getActionType()==NWParticipantEntity.OPERATION_TYPE_OPTIONAL){
	 ...
	 * 删除节点上的所有的抄送参与者
	 * */
	public void removeAllRdParticipants(NWActDef nwactdef) throws NWException{
		Vector v=this.getReadParticipant(nwactdef.openParticipantList());
