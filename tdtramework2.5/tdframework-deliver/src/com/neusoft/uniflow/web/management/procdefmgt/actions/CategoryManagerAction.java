...
public class CategoryManagerAction extends DispatchAction {
	private Log log = LogFactory.getLog(CategoryManagerAction.class);

	public ActionForward categoryManager(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		return actionMapping.findForward("categoryManager");
	}
	...
	public ActionForward addCategory(ActionMapping actionMapping,
		...
		if (inserttype.equals("category")) {
			...
			newCategory.setType(ICategoryEntry.PROCESS);
	...
	public ActionForward getCategory(ActionMapping actionMapping,
		...
		if ("root".equals(type)) {
			...
			List categories = (List) categoryEntry.openAbsoluteRootCategoryList(ICategoryEntry.PROCESS);
				...
				for (Iterator iterator = categories.iterator(); iterator
					...
					try {
						...
						if (categoryList.size() > 0) {
							j.put("leaf", false);
							j.put("iconCls", "procbranch");
						} else {
							j.put("leaf", true);
							j.put("iconCls", "procleaf");
						}
