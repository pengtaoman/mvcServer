...
public class ApplicationManagerAction extends DispatchAction {
	private Log log = LogFactory.getLog(ActTmpManagerAction.class);

	public ActionForward applicationManager(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		return actionMapping.findForward("applicationManager");
	}
	...
	public ActionForward addCategory(ActionMapping actionMapping,
		...
		if (inserttype.equals("category")) {
			...
			newCategory.setType(ICategoryEntry.APPLICATION);
	...
	public ActionForward getCategory(ActionMapping actionMapping,
		...
		if ("root".equals(type)) {
			...
			List categories = (List) categoryEntry.openAbsoluteRootCategoryList(ICategoryEntry.APPLICATION);
				...
				for (Iterator iterator = categories.iterator(); iterator
					...
					try {
						...
						if (categoryList.size() > 0) {
							j.put("leaf", false);
							j.put("iconCls", "apptmpbranch");
						} else {
							j.put("leaf", true);
							j.put("iconCls", "apptmpleaf");
						}
