...
public class AutorityPrint {
	...
	public AutorityPrint() {
	}

	public void printTree(OrgTree tree, JspWriter out)
	...
	private void writeNode(OrgNode node, JspWriter out)
		...
		for (OrgNode parent = node.getParentNode(); parent != null; parent = parent
			...
			if (parent.hasNextSibling()
					|| parent.getID() == OrgTree.ORGROLEROOT_ID)// 有后继，画|
			{
		...
		if (node.getType() == 4)
			if (node.isExpanded()) {
				buffer.append(this.formatImg(RoleSelPrinter.IMG_FOLDEROPENED));
			} else {
				buffer.append(this.formatImg(RoleSelPrinter.IMG_FOLDERCLOSED));
			}
		else if (node.getType() == 1)
			buffer.append(this.formatImg(RoleSelPrinter.IMG_ROLE));
		else if (node.getType() == 2) {
			buffer.append(this.formatImg(RoleSelPrinter.IMG_UNIT));
		} else
	...
	private String formatImgLink(String img, String nodeID) {
		return "<a href='procauthtree.do?selType=" + 2 + "&expendRoleID="
				+ nodeID + "'>" + img + "</a>";
	}

	private String formatImg(String imgName) {
		return "<IMG SRC='" + WorkflowManager.getWorkflowStylePath()
				+ "/style1/orgtree_img/" + imgName + "' border = '0'/>";
	}

	private String formatNameLink(OrgNode node) {
		StringBuffer buffer = new StringBuffer();
		if(node.getType()==0){
			buffer.append("<a href=\"javascript:selectActor('");
			...
			/*
			 * buffer.append("<a
			 * href=\"javascript:document.forms[0].selActorID.value='");
			 * buffer.append(id);
			 * buffer.append("';document.forms[0].selActorName.value='");
	...
	private void printHeader(JspWriter out) throws java.io.IOException {
		...
		sb
				.append("<table border=\"0\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"Input_text\">\n");
		// sb.append(" <tr>\n");
