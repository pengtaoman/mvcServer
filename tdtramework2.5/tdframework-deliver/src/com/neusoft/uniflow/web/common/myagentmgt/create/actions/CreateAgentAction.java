...
import com.neusoft.uniflow.web.common.myagentmgt.create.forms.CreateAgentForm;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (handleForm.getOperation().equals("ok")) {
			...
			if (agentType.equals("0")) {
				...
				// int assignertype=handleForm.getAssignerType();
				// 被代理人
				String userName = UniflowManager.getNWOrg().getPerson(userId)
						.getName();
				agent.setAssigner(userId);
				agent.setAssignerName(userName);
				agent.setAssignerType(0);// 我得代理类型只能是人。
				...
				if (categories != null && !categories.equals("")) {
					...
					for (int i = 0; i < selected.size(); i++) {
						...
						// String assignerId=handleForm.getAssignerId();
						String userName = UniflowManager.getNWOrg().getPerson(
								userId).getName();
						agent.setAssigner(userId);
						agent.setAssignerName(userName);
						agent.setAssignerType(0);
						// }else if(assignertype==1){
						// agent.setAssigner(UniflowManager.getNWOrg().getRole(assignerId).getID());
