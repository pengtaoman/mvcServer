...
public class CommonTree extends TDDispatchAction {

	/**
	 * 功能:查询、组织数据，展示CommonTree组件
	 */
	public ActionForward begin(ActionMapping mapping, ActionForm form,
		...
		MenuColl coll = (MenuColl) dao.getFuncRoleMenuInfoByRoleId(99);
		
		SystemDAO sdao = (SystemDAO) factory.getInteractionObject("systemDAO",appContext);
		SystemColl scoll = (SystemColl) sdao.getSystemCollByRole(99);
			
		int count = coll.getRowCount();
		
		ITree tree = new Tree();
		ITreeNode movies = new TreeNode("om", "权限系统", "om");
		//tree.collapse("om");
		tree.expand("om");
		tree.setRoot(movies);
		...
		for (int i = 0; i < count; i++) {
			...
			if (vo != null) {
				ITreeNode node = new TreeNode(vo.getMenuId(), vo.getMenuName(),vo.getMenuId());
		...
		return mapping.findForward("eaptreeexample");
	}
	...
	private ITreeNode getNode(MenuVO vo) {
		ITreeNode node = null;
		if (vo != null)
			node = new TreeNode(vo.getMenuId(), vo.getMenuName(), vo.getMenuName());
		return node;
	}
	...
	private void addSystemNode(ITree tree, ITreeNode node, MenuVO vo,
			SystemColl scoll) {
		ITreeNode[] nodes = tree.getNodePath(vo.getMenuId());
		String sname = getSystemName(vo.getMenuId(), scoll);
		if (nodes.length == 00 && sname != null) {
			tree.getRoot().addChild(new TreeNode(vo.getMenuId(), sname, vo.getMenuName()));
		}
	}
