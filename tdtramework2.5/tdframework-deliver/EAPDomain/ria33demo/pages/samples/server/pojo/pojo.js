	...
	function save() {
		...
		if(bool) {
			...
			unieap.Action.requestData({
				url:unieap.WEB_APP_NAME+ "/pojoTest.do?method=save", 
				sync:false,
				load:function(dc){
	...
	function query(){
		...
		unieap.Action.requestData({
			url:unieap.WEB_APP_NAME+ "/pojoTest.do?method=query", 
			sync:true,
			load:function(dc){
	...
	function queryWithCondition() {
		...
		var empNo = unieap.byId('empno').getValue();
		var empName = unieap.byId('ename').getValue();
		//ComboBox为空，取出的值为null,null在json解析时出错
		var deptNo = unieap.byId('deptno').getValue();
		if (deptNo == null) {
			deptNo = "";
		}
		...
		unieap.Action.requestData({
			url:unieap.WEB_APP_NAME+ "/pojoTest.do?method=queryWithCondition", 
			sync:true,
			load:function(dc){
				var ds=dc.getDataStore("emp");
				unieap.byId("empGrid").getBinding().setDataStore(ds);
				setFirstRowSelected();
			}
		},dc);
	...
	function checkExist() {
		...
		dc.addDataStore(store);
		unieap.Action.requestData({
			url:unieap.WEB_APP_NAME+ "/pojoTest.do?method=checkExist", 
			sync:true,
			load:function(dc){
				var ds=dc.getDataStore("emp");
			}
	...
	function onPM(store,binding){
		if(confirm("保存修改？")){
			binding.save({url:'/pojoTest.do?method=save'});
		}
	}
