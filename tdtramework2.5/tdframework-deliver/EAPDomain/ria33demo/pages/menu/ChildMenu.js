...
function createMenuList(){
	...
	result.push('</table>');	
	if(store.getRowSet().getRowCount() > maxVisibleMenus){
		dojo.style(dojo.byId("ngttabmenus"),"height",maxVisibleMenus * 25 +"px");
		dojo.style(dojo.byId("upBtn"),"display","block");
		dojo.style(dojo.byId("downBtn"),"display","block");
		keepScrollBtnStatus();
	}
	dojo.byId("ngttabmenus").innerHTML = result.join("");
...
function scrollup(){
	var tables = dojo.byId("ngttabmenus");
...
function keepScrollBtnStatus(){ 
	var tables = dojo.byId("ngttabmenus");
	if(!tables.scrollTop){
		dojo.byId("downBtn").disabled = true;
		dojo.byId("upBtn").disabled = false;
//		dojo.byId("downBtn").firstChild.style.display = "none";
		return;
	}
	else{
		dojo.byId("downBtn").disabled = false;
//		dojo.byId("downBtn").firstChild.style.display = "block";
	}
	if(tables.scrollTop + tables.clientHeight>=tables.firstChild.offsetHeight){
		dojo.byId("upBtn").disabled = true;
//		dojo.byId("upBtn").firstChild.style.display = "none";
	}
	else{
		dojo.byId("upBtn").disabled = false;
//		dojo.byId("upBtn").firstChild.style.display = "block";
	}
} 
