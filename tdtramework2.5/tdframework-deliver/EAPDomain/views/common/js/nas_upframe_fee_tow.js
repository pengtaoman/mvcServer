...
//fee_xml_info：费用串信息的路径
//div_xml_info：费用层名字信息的路径

function nas_upframe_fee_tow(fee_xml_info,div_xml_info)
{
	var if_echo = "-1";
		...
		if (temp!="")
		{
			...
			for (var i=0;i<fee_arr.length;i++)
			{
				if (fee_arr[i]!="")
				{
					...
					if (reason_text!=null && reason_text!="null" && reason_text!="NULL")
	...
	//{
		...
		if (div_name!="")
		{
			fee_div_obj = eval("window.parent.frames[1]."+div_name);
		}
		else
		{
			fee_div_obj = window.parent.frames[1].fee_div;
		}
		...
		window.parent.frames[1].fee_str_arr = fee_arr_info;		//将费用串记录下来，准备提交时组织使用
																//这个fee_str_arr数组是在上帧的nas_upframe_fee_tow.js中定义的，在uni_fee，组织费用串时用到
