...
function nas_generate_substr(sub_xml,rel_xml,group_xml,this_fild,had_str)
{
	...
	var sub_id_mark = sub_xml + "/option/Sub_service_id";
	var sub_name_mark = sub_xml + "/option/Remark";
	var sub_valid_mark = sub_xml + "/option/If_valid";
	var sub_checked_mark = sub_xml + "/option/Install_default";
	var sub_mutex_mark = sub_xml + "/option/Bus_str_mutex";
	var sub_relating_mark = sub_xml + "/option/Bus_str_relating";
	...
	if(sub_xml == 'root/Create/SubSrv1'){	
		...
		check_box_name = "check_box_create_add_info";
	}else{
		return_info += "<tr class='trType' align='center'><td width='100%' bordercolor='#E6E6E6' bgcolor='#E6E6E6' colspan='6'><p align='center'>承载业务信息</p></td></tr>";
		check_box_name = "check_box_cz";
	}
			...
			if (sub_id_t && sub_name_t && sub_valid_t && sub_checked_t && sub_mutex_t && sub_relating_t)
			{
				...
				if (sub_id_temp!="" && sub_name_temp!="" && sub_valid_temp!="")
				{
					if (sub_valid_temp == 1)
					{
						...
						if (i%4 == 1)
						{
							return_info += "<tr class='newtrType'><td width='150' bordercolor='#A2BEE1' bgcolor='#A2BEE1' height='0'><input type='checkbox'  name='"+check_box_name+"' value = '"+ sub_id_temp + "' "+sub_checked_temp+" onclick='nas_sub_click(this.value)'/>"+sub_name_temp+"</td>";
						}
						else if (i%4 == 0)
						{
							return_info += "<td width='150' bordercolor='#A2BEE1' bgcolor='#A2BEE1' height='0'><input type='checkbox'  name='"+check_box_name+"' value = '"+ sub_id_temp +"' "+sub_checked_temp+" onclick='nas_sub_click(this.value)'/>"+sub_name_temp+"</td></tr>";
						}
						else
						{
							return_info += "<td width='150' bordercolor='#A2BEE1' bgcolor='#A2BEE1' height='0'><input type='checkbox'  name='"+check_box_name+"' value = '"+ sub_id_temp + "' "+sub_checked_temp+" onclick='nas_sub_click(this.value)'/>"+sub_name_temp+"</td>";
						}
					...
					{
						return_info += "<input type='hidden'  name='"+check_box_name+"' value = '"+ sub_id_temp + "'/>";
					}
...
//合并完整串的函数：附加业务画在tab页里可以调用这个
//this_fild:tabset1_tab1
function nas_generate_unit_new(this_fild,check_box_name)
{
	fild_name = this_fild+".all('"+check_box_name+"')";
	
	if (typeof(eval(this_fild))!='undefined')
	{
		
		var the_str = "";
		var str_fild = fild_name +".length";
		if (typeof(eval(str_fild))!='undefined')
		{
			
			for (var i=0;i<eval(str_fild);i++)
			{
				check_fild = this_fild + ".all('"+check_box_name+"')["+i+"].checked";
				
				if (eval(check_fild)==true)
				{
					the_str +="1";
				}
				else
		...
		{
			check_fild = this_fild + ".all('"+check_box_name+"').checked";
			if (eval(check_fild)==true)
			{
				the_str ="1";
			}
