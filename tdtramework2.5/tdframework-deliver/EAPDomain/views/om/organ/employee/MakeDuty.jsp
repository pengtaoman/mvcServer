...
<html>
  <head>
	...
	{
		var url = webpath + '/om/EmployeeMaintanceAction.do?OperType=makeDuty';
		ec.action = url;
		ec.submit();
	}
	/**
	...
	{
		var url = path+'/views/om/organ/employee/RolePowerPage.jsp?roleId='+roleId;
		var width = 520;
		var height = 450;
    ...
    <tr>
    	<td width="100%">
			<%
				if (!size.equals("0")) {
			%>
			<ec:table items="employeeRoleColl" var="role" paginationLocation="false" rowsDisplayed="-1">
					...
					<ec:row >
					    <ec:column property="rowId" title=" " style="width:20px;text-align:center;">
					    	<c:if test="${role.usableFlag == 1 || role.adminFlag == 1}">
			            		<input type="checkbox" id="role_${role.roleId}" name="role_${role.roleId}" onclick="roleFlagChangeStat('${role.roleId}')" checked="checked" style="width:15px"/>
			            	</c:if>
						...
						<ec:column property="usableFlag" title="使用权">
							...
							<c:if test="${role.usableFlag == 1}">
								<input type="checkbox" id="u_${role.roleId}" name="uf_${role.roleId}" value="1" onclick="usebleFlagChangeStat('${role.roleId}')" checked="checked" style="width:20px"/>
							</c:if>使用权
						</ec:column>
						<ec:column property="adminFlag" title="管理权">
							...
							<c:if test="${role.adminFlag == 1}">
								<input type="checkbox" id="a_${role.roleId}" name="af_${role.roleId}" value="1" onclick="adminFlagChangeStat('${role.roleId}')" checked="checked" style="width:20px"/>
							</c:if>管理权
						</ec:column>
				    </ec:row>
				</ec:table>
