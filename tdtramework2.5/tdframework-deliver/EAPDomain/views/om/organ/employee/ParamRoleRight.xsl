...
<HEAD>			
		<STYLE>
			...
			<SCRIPT language="javascript" src="{/root/path}/views/om/common_css_js/select_default.js"/>
			<SCRIPT>
		<xsl:comment><![CDATA[
			...
			function writeBranch(){
				...
				branchString +='<input type="checkbox" name="ParamTableData" Index="'+this.id+'" level="'+this.level+'" readonly="true"';
				branchString +='isCheck="'+this.isCheck+'"/>' + this.text;
				branchString += '</span>';		
			...
			function writeLeaf(){
				var leafString = '<input type="checkbox" name="ParamTableData" Index="'+this.link+'" level="'+this.level+'" readonly="true"';
				leafString += 'isCheck="'+this.isCheck+'" />';
			...
			var isModify = false;
		  	...
		  	function update_sys(){		  						
				if(checkChange()){
					...
					window.parent.frames[2].myform.action="dataRoleManage.do";
			...
			function query(){
				...
				document.myform.oprType.value="queryEmpPower";
				document.myform.tableName.value=document.myform.tableCol.value;
	...
	<form method="POST"  name="myform">
			...
			<input type="hidden" value="" name="employeeId"><xsl:attribute name="value"><xsl:value-of select="/root/employeeId"/></xsl:attribute></input>
			...
			<xsl:for-each select="ParamTable">
			   <tr class="trType">		   
				...
				<td bordercolor="#E6E6E6" bgcolor="#F0F0F0">
					<input type="checkbox" name="ParamTableData" class="checkBoxType"  disabled="true">
