...
//增加操作
	...
	if( document.myform.OperType.value == 'add'){
		if (document.myform.OperFlag.value == '1'){
			if (window.opener.parent.employeelist.document.getElementById('ec')){
				window.opener.parent.employeelist.document.getElementById('ec').submit();
			} else
			{
...
//修改操作
	if(document.myform.OperFlag.value == '1'&& document.myform.OperType.value == 'modify'){
		...
		//parent.myform.bSave.disabled=false;
		//判断是否修改了职务,如果修改了职务,需要重新装载页面 renh add
		/**
...
//删除操作
	if(document.myform.OperFlag.value == '1'&& document.myform.OperType.value == 'delete'){
        ...
        if(OrganKind =="area"){
            ...
            parent.employeelist.location.href = "EmployeeQueryAction.do?"+"&OrganKind='duty'"+"&BelongArea="+BelongArea+"&AreaId="+BelongArea+"&OrganId=" + OrganId + "&DutyId= " +DutyId + "&OperType=employeeList";
        }
...
//赋权操作
  ...
  <form name="myform">
    <input type="hidden" name="EmployeeId"/>
		...
		<!--页面操作-->
		<TEXTAREA cols="1" rows="1" name="errorInfo" style="display: none"><%=message%></TEXTAREA>
