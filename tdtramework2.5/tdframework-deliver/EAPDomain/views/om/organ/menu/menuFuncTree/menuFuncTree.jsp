...
<head>
   ...
   <link_rel="stylesheet_href="<%=webpath%>/unieap/css/tab/unieapTab.css"_type="text/css">
</link>
...
function customDefineparas(treeFlag,id){
	var nodeValue = getNode(treeFlag,id).nodevalue;
	alert(nodeValue);
	
    return "&roleId="+'<%=roleId%>'+"&operType="+'<%=operType%>'+"&employeeId="+'<%=employeeId%>';
}
  ...
  function init(){
   ...
   function clicknodeforexpandorcollapse(treeflag,nodeid){
    ...
    function childMenuPage(nodeid){
    	...
    	if('<%=operType%>' == 'viewTree'){
			parent.tab_page_2.location.href='<%=webpath%>/om/roleManage.do?method=openChildPage&nodeId='+nodeid+'&roleIdValue='+roleId+'&ifUseBox=false';
			nextPage("tab",'');
   		}else{
			parent.tab_page_2.location.href='<%=webpath%>/om/roleManage.do?method=openChildPage&nodeId='+nodeid+'&roleIdValue='+roleId;
			nextPage("tab",'');
   		}
...
<div id="treediv">
<unieap:tree  tree='menuTree' includeRootNode="true" readOnly="true" mode="ALLLOAD" needCheckBox='<%=needCheckBox%>' textClass="TreeNode" 
