			...
			var isModify = false;
		  	function init(){		  	    
				...
				if(document.myform.ParamTableData != undefined){	
					document.myform.checkall.disabled=false;	
				}
						  	    var view = parent.ParamRoleList.myform.view.value;
		  	    if(view == "true"){
		  	    	setReadOnly();
		  	    }
		  	}			
...
<xsl:template match="/root">
	...
	<table width = "440" border="1" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
			...
			<tr class="trType">
				...
				<input type="checkbox" name="checkall" onclick="check_all();" disabled="true"></input> 全部选中
				</td>
			</tr>
			<tr class="trType">
