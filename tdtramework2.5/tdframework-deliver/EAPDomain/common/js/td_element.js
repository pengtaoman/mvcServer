var Element={
	...
	showhideCore : function(att,attarg,elems){
				if (typeof(elems) == 'string' || !elems.length){
					elems=[].concat(elems);
				}
	...
	hide: function() {
				...
				Element.showhideCore(att,attarg,arguments);
		  },
	hideb: function() {
	...
	toggle: function() {
				...
				for (var i = 0; i < arguments.length; i++) {
				  ...
				  attarg=Element.visible(element) ? 'none' : '';
				  Element.showhideCore(att,attarg,element);
				}
		},
	...
	toggleb: function() {
				...
				for (var i = 0; i < arguments.length; i++) {
				  var element = $(arguments[i]);
				  attarg=Element.visibleb(element) ? 'hidden' : '';
				  Element.showhideCore(att,attarg,element);
				}
		},
	...
	flashAction: function(){
				if (!Element.flashElement){ return ;	}
				Element.toggleb.apply(this,[].concat(Element.flashElement) );
		},
	flash: function(elements,ftimes,delay){
			Element.flashElement=elements;
			ftimes=ftimes?ftimes:2;
			delay=delay?delay:130;
			var timeout=0
			for (var i=0;i<ftimes;i++ ){
				timeout+=delay;
				window.setTimeout(Element.flashAction,timeout);
				timeout+=delay;
				window.setTimeout(Element.flashAction,timeout);
			}
			window.setTimeout(Element.flashOver,timeout+delay);

		},
	flashOver: function(){
			Element.flashElement=null;
		},
