...
function Tree_treeView()
{
	...
	this.add=function add(relate_ID,nOption,nIndex,text,hint,status,target,descript,page,chief,img,role,img1,img2){
		if(nOption==Tree_ROOT)	{
			return this.container.addChild(nIndex,text,hint,status,target,descript,page,chief,img,role,img1,img2);
		}
		if(nOption==Tree_CHILD){
			var parent=this.getNode(relate_ID);
			if(parent==null) return null;
			return parent.addChild(nIndex,text,hint,status,target,descript,page,chief,img,role,img1,img2);
		}
		if(nOption==Tree_SIBLING){
			if(relate_ID==this.container.id) return null;
			var node=this.getNode(relate_ID);
			if(node==null) return null;
			return node.addSibling(nIndex,text,hint,status,target,descript,page,chief,img,role,img1,img2);
		}
		return null;
	}
...
//-----------------------------
function Tree_node(text,hint,status,target,descript,page,chief,img,role,img1,img2,action)
{
	...
	this.descript=typeof(descript)=="string"?descript:"";
	this.page=typeof(page)=="string"?page:"";
	this.chief=typeof(chief)=="string"?chief:"";
	this.img=typeof(img)=="string"?img:"";
	...
	//------------------
	this.addChild=function addChild(index,text,hint,status,target,descript,page,chief,img,role,img1,img2){
		var action=new Tree_action(0,0,0,0,this.action.treeViewIndex);
		var node=new Tree_node(text,hint,status,target,descript,page,chief,img,role,img1,img2,action);
		this.add(node,index);
		return node;
	}
	...
	this.addSibling=function addSibling(index,text,hint, status,target,descript,page,chief,img,role,img1,img2){
		if(this.parent==null)return null;
		if(index==Tree_PREV) index=this.index;
		if(index==Tree_NEXT) index=this.index+1;
		return this.parent.addChild(index,text,hint,status,target,descript,page,chief,img,role,img1,img2);
	}
...
function Tree_buildTree(node,treeView)//init treeView from array
{
	...
	while( index <= nLen )
	{
		...
		case Tree_const_file:			
			...
			nodeChild=new Tree_node(node[index+1],node[index+2],node[index+3],node[index+4],node[index+4],node[index+4],node[index+4],node[index+4],node[index+4],node[index+4],node[index+4],action);
		...
		case Tree_const_folder:	
			...
			nodeChild=new Tree_node(node[index+1],node[index+2],node[index+3],node[index+4],node[index+5],node[index+6],node[index+7],node[index+8],node[index+9],node[index+10],node[index+11],action);
			if(count==0) treeView.container.add(nodeChild,Tree_LAST);
			else nodeParent.add(nodeChild,Tree_LAST);
