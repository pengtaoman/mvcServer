...
dojo.declare("unieap.layout.HBoxContainer",[unieap.layout.Container,dijit._Templated],{
	...
	doLayout:function(c){
		...
		availableWidth=availableWidth-this.paddings.left-(this.margins.left+this.margins.right)*(nodes.length);
		...
		for(var i=0,l=noFixed.length,node;i<l;i++){
			...
			if(availableWidth>0){
				...
				dojo.style(node,'width',(width-parseInt(node.style.borderLeftWidth||0)-parseInt(node.style.borderRightWidth||0))+"px");
			}else{
		...
		for (var i = 0, l = nodes.length; i < l; i++) {
			...
			if (i == 0) {
				...
				node.style.left =(dojo.style(prevNode, 'left') + prevNode.offsetWidth + this.margins.right + this.margins.left) + "px";
			}
			//处理align处理
	...
	_getWidth:function(node){
		...
		return {width:parseInt(node.offsetWidth,10)};
	...
	_doVeticalLayout:function(node){
		switch(this.align){
			//居上显示时,top值为this.padding.top+this.margins.top
			case "top":
			...
			//居中显示
			case 'middle':
				var distance=(this.containerNode.offsetHeight-node.offsetHeight)/2.0;
			...
			//居下显示
			case "bottom":
				node.style.bottom=(this.paddings.bottom+this.margins.bottom)+"px";
				break;
		}
	},
