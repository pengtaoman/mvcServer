	...
	function init(){
		...
		dataStore.setRowSetName("emp");
		dataStore.addStatistic("attr_empno","max");
		dataStore.addStatistic("attr_hiredate","min");
		dataStore.addStatistic("attr_sal","avg");
		dataStore.addStatistic("attr_sal","sum");
	...
	unieap.Action.requestData({
			url:unieap.WEB_APP_NAME+ "/DCTest.do?method=testSave", 
			parameters:{"asynLoadStatistics":true},
			sync:true,
			load:function(dc){
