dojo.provide("unieap.layout.BorderPaneForSubOffer");
dojo.require('unieap.layout.Container');

dojo.declare("unieap.layout.BorderPaneForSubOffer",[unieap.layout.Container,dijit._Templated],{ 
	/**
     * @declaredClass:
	...
	templateString:"<div class=\"borderPaneOuter\">"+
						...
						"<div dojoAttachPoint='paneNode' class=\"borderPane\">"+
							...
							"</div>"+
							"<div dojoAttachPoint='containerNode' style=\"overflow:hidden;width:100%;position:relative;\">"+
							"</div>"+
						"</div>"+
					"</div>",
	...
	layoutInner: function() {
		...
		if(this.region == 'top') {
			this.paneNode.style.height = this._adjustSize(this.domNode.clientHeight - this.splitLineSize - this.borderSize - this.diffHeight);//splitLine width:5px  paneNode border:2px
			dojo.place(this.splitNode,this.domNode,'last');
		} else if(this.region == 'bottom'){
			this.paneNode.style.height = this._adjustSize(this.domNode.clientHeight - this.splitLineSize - this.borderSize - this.diffHeight);
			...
			this.paneNode.style.height = this._adjustSize(this.domNode.clientHeight - this.borderSize - this.diffHeight);
			this.wrapNode.style.height = this._adjustSize(this.domNode.clientHeight - this.borderSize - this.diffHeight);//如果收缩条显示，需要设置收缩条高度
		} else if(this.region == 'center') {
			this.paneNode.style.height = this._adjustSize(this.domNode.clientHeight - this.borderSize - this.diffHeight);
		}
		this.containerNode.style.height = this._adjustSize(this.paneNode.clientHeight - this.titleSize - 30);
