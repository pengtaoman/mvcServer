...
function addSelect(select)
{
  ...
  if(document.all)
    actorSelect=window.frames["actorsel"];
  else
    actorSelect=document.getElementById("actorsel").contentWindow;

  var partID = actorSelect.getSelectedID();
  var flag = true;
  if(partID=="")
  {
    warning_info.innerHTML ="<%=MessageUtil.getString("workflow.workitem.operation.adhoc.choice.checked",request.getSession())%>";
  ...
  var partName = actorSelect.getSelectedName();
  var partType = actorSelect.getSelectedType();
...
<uniflow:p_body onload="javascript:afterload()" width="93%">
...
<uniflow:p_content_comm_wrapper width="100%">
  ...
  <uniflow:p_content_table>
	...
	<td class="main_label_td" valign="top" height="100%" nowrap>
	  <iframe scrolling="yes" id="actorsel" width="250" height="380px" frameborder="1" src="oneactorsel.do?selType=1&treeType=1"></iframe>
	</td>
