...
function reload(){
  location.href = "agentwork.do?state="+"<%=state%>";
}
...
function adhoc_onclick(){
    ...
    var adhoc = getSelectedItemValue("ADHoc");
...
function complete_onclick(){
  ...
  if(needCommitRole=="true"){
    ...
    document.agentWorkListForm.operation.value = "complete";
    document.agentWorkListForm.action = "agentwork.do";
    document.agentWorkListForm.submit();
  }
}
...
function itemChanged()
{
  ...
  if(agentWorkListForm.selectedItem){
    if(agentWorkListForm.selectedItem.length)
    {
	for (i = 0; i < agentWorkListForm.selectedItem.length; i++) {
	  rb = agentWorkListForm.selectedItem[i];
	  if (rb.checked) {
	    checkedItem = rb;
	    isADHoc = agentWorkListForm.ADHoc[i].value;
	    isReasign = agentWorkListForm.reassignedWI[i].value;
	    curState = agentWorkListForm.curState[i].value;
	    break;
	  }
	}
    }
    ...
    {
	checkedItem = agentWorkListForm.selectedItem;
	isADHoc = agentWorkListForm.ADHoc.value;
	isReasign = agentWorkListForm.reassignedWI.value;
	curState = agentWorkListForm.curState.value;
    }
...
<uniflow:m_form action="agentwork.do">
		...
		<table height="28" border="0" cellpadding="0" cellspacing="0"class="main_label_table">
           <tr><td height="25" valign="middle" nowrap class="main_label_td">
              <select onchange="javascript:changeto('allwork.do')" 
					...
					<option value="agent" selected><bean:message bundle="uniflow" key="workflow.workitem.list.agent" /></option>
   ...
   <uniflow:m_table style="main_list">
	 ...
	 <uniflow:m_order_th name="name"    value="workflow.workitem.name"
	    ...
	    orderKey='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("agentWorkListForm")).getOrderBy()%>' 
	    ascending='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("agentWorkListForm")).isAscending()%>'/>	 
   ...
   <uniflow:order_th value="workflow.workitem.procinstname" /> 
	 <logic:iterate id="workitem" name="agentWorkListForm" property="list" type="com.neusoft.uniflow.api.handler.NWWorkItem" indexId="index">
	...
	<td valign="middle" class="main_list_td"><%=CommonInfoManager.getProcInstName(workitem.getWorkItemID())%>&nbsp;</td>
		...
		<tr><td>
			<uniflow:pageCtr
				curPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("agentWorkListForm")).getCurrentPage()%>'
				maxPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("agentWorkListForm")).getPagesCount()%>'
				total='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("agentWorkListForm")).getTotal()%>' />
		</td><td align="right" >
		...
		<%}else if(operation.equals("all")){ %> 
            ...
            <uniflow:button id="showProcessto" name="流向" action="javascript:nextact_onclick()"/>
        <%} %>
		</uniflow:m_button_table>
		</td></tr>
        </uniflow:m_table>
