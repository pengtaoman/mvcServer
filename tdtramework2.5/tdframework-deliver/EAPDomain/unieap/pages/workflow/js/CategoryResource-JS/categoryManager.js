 ...
 *  */
var Action_GetCategory=WEB_APP_CONTENT+"/m_category.do?method=getCategory";
var Action_AddCategory=WEB_APP_CONTENT+"/m_category.do?method=addCategory";
var Action_DeleteCategory=WEB_APP_CONTENT+"/m_category.do?method=deleteCategory";
var Action_ModifyCategory=WEB_APP_CONTENT+"/m_category.do?method=modifyCategory";
...
var realRemoveText='您确认要删除分类“';												 //真的要删除
   ...
   //当合拢一个节点时，让此节点成为被选中的节点，使得子节点不能被误编辑
    ...
    croot.expand();
  	...
  	function onNodeClick(){
		...
		{        				   		
  		...
  		window.open(contextPath+"/categoryresource.do?parentIdStore="+parentIdStore+"&deleteId=all","categoryResource"); 
  	}
	...
	function delSuccess(o)
	{
		...
		if(jsonraw[0].result=='1')
		{
			...
			ctree.getSelectionModel().select(parentnode);
			onNodeClick(); 	
    ...
    function removeNode(){
        ...
        if(n && n.attributes.allowDelete){
        	...
        	Ext.MessageBox.show({
          		 title:remindText,
          		 msg: realRemoveText+n.attributes.text+'”吗?',
    ...
    function removeResource(){
        	...
        	Ext.MessageBox.show({
          		 title:remindText,
          		 msg: realRemoveText+proc_tmp_id+'”吗?',
           		 buttons: Ext.MessageBox.YESNO,
