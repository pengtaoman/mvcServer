...
function unRecSelected(evt,id,path ,style){
...
function byBranchRecSelected(evt,recid,name,x,y,l,path,parentActInstID){
	  currentBranchID=recid;
	  currentParallelID=parentActInstID;
	  addMainMenu(path);
      var obj = document.getElementById("rec"+recid);
      actRecColor=obj.getAttribute("style");
      obj.setAttribute("style","fill:#cccccc;stroke:#cccccc; stroke-width: 1; stroke-opacity: 5;");       
...
function unBranchRecSelected(evt,id,path){
	  currentBranchID="";
	  currentParallelID="";
	  addMainMenu(path);
      var obj=document.getElementById(id);
...
function fileLoaded(data){
	var xmlDoc=data.content;
	var newItems=getBranchItems();
	if(newItems!="")
	{
		xmlDoc=xmlDoc.substr(0,xmlDoc.length-9);
		xmlDoc=xmlDoc+newItems+"\n</menu>";
	}
     if(data.success){
         var newMenuRoot=parseXML(xmlDoc,contextMenu)
