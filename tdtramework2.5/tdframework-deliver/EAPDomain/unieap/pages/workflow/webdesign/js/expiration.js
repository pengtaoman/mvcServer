...
function setParentValue(){
	...
	window.opener.document.manualNodeForm.exActionName.value =sendExAn;//send Delay Time
	window.opener.document.manualNodeForm.exAlertActionName.value =sendExAAN;//send Delay Time
	window.opener.document.manualNodeForm.duration.value =sendDelayTime;//send Delay Time
	window.opener.document.manualNodeForm.alertDuration.value=sendAlertTime;//send Alert Time
	window.opener.document.manualNodeForm.alertActionInterval.value=sendIntervalTime;//send alarm Interval Time
	window.opener.document.manualNodeForm.varOrDur.value=sendDelayVar;//send delay radio variable choice
	window.opener.document.manualNodeForm.alertVarOrDur.value=sendAlertVar;//send alerm radio variable choice
	window.opener.document.manualNodeForm.actionType.value=sendDelayType;//send delay type
	window.opener.document.manualNodeForm.alertActionType.value=sendAlertType;//send alarm type
	window.opener.document.manualNodeForm.actionApplication.value=sendDA;//send Delay application program
	window.opener.document.manualNodeForm.alertActionApplication.value=sendAA;//send Alarm application program
	window.opener.document.manualNodeForm.variable.value=sendVariable;//send delay variable select choice
	window.opener.document.manualNodeForm.alertVariable.value=sendAlertVariable;//send alarm variable select choice
	window.opener = null;
	window.close();
}
...
function displayDiv(){
   ...
   $("delayApplication").value=window.opener.document.manualNodeForm.exActionName.value;
   $("alertApplication").value=window.opener.document.manualNodeForm.exAlertActionName.value;
}
function createSelect(){
	var str=window.opener.document.getElementById("variablesManualNode").value;
...
function displayDelayApp(){
    if($('delayApp').checked==true){
		$('application1').style.display="block";
         ...
         if($('application1').style.display=="block"){
...
function variableSelect1(){
    ...
    for(i=0;i<obj.length;i++){
		if(obj[i].innerHTML==window.opener.document.manualNodeForm.variable.value){
			obj[i].selected=true;
...
function variableSelect2(){
    ...
    for(i=0;i<obj.length;i++){
		if(obj[i].innerHTML==window.opener.document.manualNodeForm.alertVariable.value){
			obj[i].selected=true;
