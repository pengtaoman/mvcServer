...
var nameText='名称';																	 //名称
var titleText='标题';																 //标题
var appIdArray = new Array();//当前选定的树节点
var PosX;
var PosY;
...
Ext.onReady(function(){   
    ...
    var appIds = '<%=request.getParameter("applicationId")%>';
    if (appIds.length > 0) {
    	if (appIds.indexOf(',') != -1) {
    		appIdArray = appIds.split(',');
    	} else {
    		appIdArray =new Array(appIds);
    	}
    }
    ...
    ctree = new tree.TreePanel('cbody', {
        ...
        dataUrl:'<%=request.getContextPath()%>/appNew.do?method=getCategory&name='+'<%=request.getParameter("applicationPageName")%>',
   ...
   //当合拢一个节点时，让此节点成为被选中的节点，使得子节点不能被误编辑
	...
	ctree.on('click',function(node ,checked){
  		var parentIdStore;			
  		var sm = ctree.getSelectionModel();	  	 
  		var n = sm.getSelectedNode();
  		
     	CategoryManager_selectedNode=node;
...
function calculateSelectedApps() {
    appId = "";
    appName = "";
	calculateCheckedNode(ctree.root);
	if (appId.length > 0) {
...
//获取选中节点
function calculateCheckedNode(node) {
    //判断根节点是否选中
    if (node.leaf && node.getUI().checkbox.checked) {
        appId += node.attributes.category_id + ",";
        appName += node.attributes.text + ",";
    }
    var treeNodes = node.childNodes;
    //遍历根节点下的所有子节点
    for (var i = 0; i < treeNodes.length; i++) {
    	calculateCheckedNode(treeNodes[i]);
    }
}
...
function commitValue(){
	...
	switch(name){
		...
		case "openManualNodeApplication": window.opener.document.manualNodeForm.applicationName.value = appName;
		         window.opener.document.manualNodeForm.application.value = appId; break;
        default:window.opener.document.getElementById(name + 'Action').value = appId;
				window.opener.document.getElementById(name + 'Text').value = appName;
	}
	window.close();
}

function initTreeChecked(){
	for(i=0;i<appIdArray.length;i++){
		var applicationId=appIdArray[i];
		if(ctree.getNodeById(applicationId)!=null){
			ctree.getNodeById(applicationId).attributes.checked=true;
		}
	}
