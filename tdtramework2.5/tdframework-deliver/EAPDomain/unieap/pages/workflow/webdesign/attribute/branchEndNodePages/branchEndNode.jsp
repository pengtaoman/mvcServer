...
<%@page import="com.neusoft.uniflow.web.webdesign.AttributeOperation.forms.branchEndNodeForms.BranchEndNodeForm;"%>
...
<head>
    ...
    function LoadName(){ 
    	if(document.all){
          ...
          if(action == "close"){
             var xmlStr = document.branchEndNodeForm.xmlStr.value;
 	...
 	if(trim(document.branchEndNodeForm.name.value)==""){
		document.branchEndNodeForm.name.focus();
	 	alert("名称不可以为空")
	 	document.branchEndNodeForm.name.value="";
		return false;
	}
  	 	document.branchEndNodeForm.action.value="submit";
     	document.branchEndNodeForm.submit();
	}   
    </script>
</head>
...
<body  onLoad="LoadName()"><br>
  ...
  <html:form action="toBranchEnd.do" >
   <div class="main_label_outline" style="width:570px">
		  	<table style="width:550px" class="main_label_table">
		  		<tr>
					...
					<td>
						<html:text property="id" readonly="true" style="width:400px;"
							...
							value='<%=((BranchEndNodeForm) request
										.getAttribute("BranchEndNodeForm"))
										.getId()%>'>
						</html:text>
					</td>
				</tr>
				<tr>
					...
					<td>
					<html:text property="name" style="width:400px;" styleClass="input_underline"  value='<%=((BranchEndNodeForm)request.getAttribute("BranchEndNodeForm")).getName()%>'>
					</html:text>*
					</td>
				</tr>
				<tr>
					<td valign="top"  style="width:90px"><bean:message bundle="uniflow" key="workflow.webdesign.describe"/></td>
					<td><html:textarea property="desc" style="width:400px;" rows="5" styleClass="input_text"  value='<%=((BranchEndNodeForm)request.getAttribute("BranchEndNodeForm")).getDesc()%>'/></td>
				</tr>
			</table>
			<table style="width: 550px" class="main_label_table">
				<tr>
					...
					<td >
						<html:text property="extendProperties" style="width:400px;" value='<%=((BranchEndNodeForm) request
									.getAttribute("BranchEndNodeForm")).getExtendProperties()%>'/>
					</td>
				</tr>
			</table>
	</div>	
