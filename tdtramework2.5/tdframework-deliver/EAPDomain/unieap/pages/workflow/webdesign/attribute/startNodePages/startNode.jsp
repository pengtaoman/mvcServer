...
<%@page import="com.neusoft.uniflow.web.webdesign.AttributeOperation.forms.startNodeForms.StartNodeForm;"%>
...
<head>
    ...
    function LoadName(){ 
    	if(document.all){
          ...
          if(action == "close"){
             var xmlStr = document.startNodeForm.xmlStr.value;
 	...
 	if(trim(document.startNodeForm.name.value)==""){
		document.startNodeForm.name.focus();
	 	alert("名称不可以为空")
	 	document.startNodeForm.name.value="";
		return false;
	}
  	 	document.startNodeForm.action.value="submit";
     	document.startNodeForm.submit();
	}   
    </script>
</head>
   ...
   <div class="main_label_outline" style="width:570px">
		  	<table style="width:550px" class="main_label_table">
		  		<tr>
					...
					<td>
						<html:text property="id" readonly="true" style="width:400px;"
							...
							value='<%=((StartNodeForm) request
										.getAttribute("StartNodeForm"))
										.getId()%>'>
						</html:text>
					</td>
				</tr>
				<tr>
					...
					<td>
					<html:text property="name" style="width:400px;" styleClass="input_underline"  value='<%=((StartNodeForm)request.getAttribute("StartNodeForm")).getName()%>'>
					</html:text>*
					</td>
				</tr>
				<tr>
					<td valign="top"  style="width:90px"><bean:message bundle="uniflow" key="workflow.webdesign.describe"/></td>
					<td><html:textarea property="desc" style="width:400px;" rows="5" styleClass="input_text"  value='<%=((StartNodeForm)request.getAttribute("StartNodeForm")).getDesc()%>'/></td>
				</tr>
			</table>
			<table style="width: 550px" class="main_label_table">
				<tr>
					...
					<td >
						<html:text property="extendProperties" style="width:400px;" value='<%=((StartNodeForm) request
									.getAttribute("StartNodeForm")).getExtendProperties()%>'/>
					</td>
				</tr>
			</table>
	</div>	
