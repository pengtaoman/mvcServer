...
<title>分类管理</title>
 	...
 	<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/unieap/pages/workflow/stylesheet/vartmpresourcemgt/varTmpManager.css" />
	<%@include file="ext-js.jsp" %> 
    <script type="text/javascript" src="<%=request.getContextPath()%>/unieap/pages/workflow/js/VarTmpResource-JS/varTmpManager.js"></script>	
    ...
    <% String addNewNodeText="新增一个节点"; 													//新增一个节      									
      ...
      <script language="javascript">     		
      			...
      			{       
	      			...
	      			if(length>limitNum)
	      			{   
	      				Ext.Meg.msg(text+"<%=tooLongText%>","<%=longBeyondText%>"+limitNum+"<%=byteLimitText%>");	      						
	      				return false;
	      			}		
	      			else
	      			{	      				
...
<br><br><!-- 树 -->
 <div id="addcategory-dlg" class="add-dialog" style="position: relative">
...
<script language="javascript">
			         ...
			         /*	IE可以通过设置opropertychange来捕获值改变事件 */
					if(Ext.isIE)
					{					
	      				var categorynameObj=document.getElementById('categoryname');
	      				categorynameObj.attachEvent("onpropertychange",this.categoryNameLimit);
					}
					/*如果为firefox就不能使用*/
	      			else
	      			{  				
	      				document.getElementById('categoryname').addEventListener("input",this.categoryNameLimit,false);
