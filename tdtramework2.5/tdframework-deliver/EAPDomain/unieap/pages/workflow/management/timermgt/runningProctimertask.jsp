...
<html>
		...
		<script type="text/javascript">
			...
			function query(){
				...
				var ddc=unieap.Action.requestData({
						url:unieap.WEB_APP_NAME+ "/loadTimerProcess.do?type=0&method=loadRunningTimertaskProcess",
						sync:true,
						load:function(dc,ds){
						}
					},dc);
			...
			function doReset(){
	            	...
	            	unieap.Action.requestData({
							...
							parameters:{selectedID:id,type:"procID",state:"running"},
							sync:true
						});
					query();
            ...
            function showDetail(){
            	...
            	if(index.length > 0){
	            	...
	            	var url = unieap.WEB_APP_NAME + "/runningTimerTaskProcessDetail.do?type=0&procInstID=" + selectedProcID;
	            	window.open(url);
            	}
            }
    ...
    <body class="unieap" style="visibility:hidden;">
		<div id="grid" jsId="grid" dojoType="unieap.grid.Grid" width="100%" height="500px"
					...
					<fixed>
						<cell width="150px" label="实例名称" name="name" styles="text-align:center"></cell>
					</fixed>
					<header>
						<cell width="150px" label="模板名称" name="templet_name" styles="text-align:center"></cell>
						<cell width="120px" label="流程状态" name="current_state" styles="text-align:center"></cell>
						<cell width="150px" label="业务分类" name="category" styles="text-align:center"></cell>
						<cell width="150px" label="开始时间" name="start_time" styles="text-align:center"></cell>
						<cell width="150px" label="完成时间" name="complete_time" styles="text-align:center"></cell>
					</header>
					 <toolbar paging="{parameters:{'method':'loadRunningTimertaskProcess','type':'0'},url:'/loadTimerProcess.do'}" individual="false" export="false" print="false">
		            	...
		            	<button dojoType="unieap.form.Button" label=" 详细 " onClick="showDetail()"></button>
		            </toolbar> 
		</div>
		</body>
</html>