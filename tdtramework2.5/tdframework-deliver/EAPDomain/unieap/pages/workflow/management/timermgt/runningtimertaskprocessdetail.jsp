...
<html>
		...
		<script type="text/javascript">
			...
			function query(){
				...
				var ddc=unieap.Action.requestData({
						url:unieap.WEB_APP_NAME+ '/LoadErrorTimertasks.do?method=loadRunningTimertasks&type=<%=request.getParameter("type")%>',
						sync:true,
    ...
    <body class="unieap" style="visibility:hidden;">
		<div id="grid" jsId="grid" dojoType="unieap.grid.Grid" width="100%" height="500px"
					...
					<fixed>
						<cell width="150px" label="模板名称" name="procName" styles="text-align:center"></cell>
					</fixed>
					<header>
						<cell width="150px" label="实例名称" name="instName" styles="text-align:center"></cell>
					 ...
					 <toolbar paging="{parameters:{procInstID:'<%=request.getParameter("procInstID")%>','type':'<%=request.getParameter("type")%>'},url:'/LoadErrorTimertasks.do?method=loadRunningTimertasks'}">
