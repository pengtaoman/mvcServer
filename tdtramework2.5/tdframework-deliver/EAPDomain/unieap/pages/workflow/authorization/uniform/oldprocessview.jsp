...
function setActAppID(actid){
    var URL = "<%=request.getContextPath()%>/oneappsel.do?activityid="+actid+"&actiontype=fm";
   	document.all.frame.src=URL;
}
function OpenFormAuthoritySetting(actid,actname,url,type,parti){
  location.href = "<%=request.getContextPath()%>/unieap/pages/workflow/authorization/uniform/formauthWindow.jsp?activityid="+actid+"&formid="+url+"&participants="+parti+"&activityname="+actname;  
}
function winZoomIn(){
...
function refresh(){
  location.href = "<%=request.getContextPath()%>/processAuthor.do?processid=<%=request.getParameter("processid")%>&version=<%=request.getParameter("version")%>";
}
...
<uniflow:m_form action="processAuthor.do">
...
<tr><td bgcolor="#EEEEEE"><table>
	...
	<tr><td align="center">
		<div style="overflow:auto;background-color:#FFFFFF" id="div"><embed
			...
			src='<%=request.getContextPath()%>/displaySvg?print=oldaprocdef'>
		</embed></div>
	</td></tr>	
</table></td>
