...
<SCRIPT language="JavaScript" src="unieap/pages/form/jslib/list.js"></SCRIPT>
...
<script language="javascript">
	...
	function turnTo(total) {
		...
		if (flag) {
			location.href="<%=request.getContextPath()%>/privilegeturnPage.do?page="+num+"&isNowSession='true'";
		}
		else {
	...
	returnObj.name = "";
	function showQueryDialog(frm) {
		window.showModalDialog('<%=request.getContextPath()%>/unieap/pages/form/query.jsp', returnObj, "dialogWidth=466px;dialogHeight=216px;center:yes;resizable:no;scroll:no;status:no;");   
		if(returnObj.value != undefined)
			location.href="<%=request.getContextPath()%>/privilegeFormList.do?"+returnObj.value+"&isNowSession='true'";
		else if(returnObj.name == "") {
			// do nothing
		}
		else
			location.href="<%=request.getContextPath()%>/privilegeFormList.do?"+"isNowSession='true'";
	}

	function showAllForms() {
		location.href="<%=request.getContextPath()%>/privilegeFormList.do?";
	}

	function doSort(sortColumnName) {
		location.href="<%=request.getContextPath()%>/privilegeFormList.do?sortColumnName="+sortColumnName+"&isNowSession='true'";
	}

	function toOrg(){
		var formID = getSelectedItemID();
		if(formID == null){
			var msg = '<%=MessageUtil.getString("form.privilege.toorg",session)%>';
			/*var styles="dialogWidth:426px;dialogHeight:240px;help:no;status:no;resizable:no"
		...
		location.href="<%=request.getContextPath()%>/unieap/pages/form/formprivilege.jsp?formID="+formID;
		//location.href="<%=request.getContextPath()%>/roleFormList.do?roleId=qqq"; 
	}

	function goToPage(pageCode){
		location.href="<%=request.getContextPath()%>/privilegeturnPage.do?page="+pageCode+"&isNowSession='true'";
	}
...
<form action="<%=request.getContextPath()%>/privilegeturnPage.do" method="POST">
</form>
<form>
</form>
...
<table cellspacing="0" class="main_list" style="margin-top:40px">
  <tr >
    ...
    <td width="24%" onclick="doSort('description')" class="main_list_th" style="cursor:hand" nowrap>
  ...
  <tr>
    ...
    <td align="right">
    	<table class="button_table" cellspacing="0">
      	<tr align="right">
      		<td class="showall_button_td">
      			<input type="button" name="showall" value='<%=MessageUtil.getString("form.privilege.showall",session) %>' class="button_normal" onclick="javascript:showAllForms()"/>&nbsp;
      		</td>
      		<td class="query_button_td" >
      			<logic:equal name="pageController" property="hasData" value="true">
        			<input type="button" name="query" value='<%=MessageUtil.getString("form.privilege.query",session) %>' class="button_normal" onclick="javascript:showQueryDialog(document.form)"/>
        		</logic:equal>
        	...
        	<td class="button_td">
        		<logic:equal name="pageController" property="hasData" value="true">
        			<input type="button" name="setprv" value='<%=MessageUtil.getString("form.privilege.privilege",session) %>' class="button_normal" onclick="javascript:toOrg()"/>
        		</logic:equal>
        		<logic:equal name="pageController" property="hasData" value="false">
        			<input type="button" name="setprv" value='<%=MessageUtil.getString("form.privilege.privilege",session) %>' class="button_normal" />
        		</logic:equal>
        	</td>
      	</tr>
    	</table>
