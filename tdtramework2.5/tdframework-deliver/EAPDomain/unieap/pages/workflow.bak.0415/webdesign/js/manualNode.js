...
function initPage() {
	if (document.all) {
		self.resizeTo(635, 430);
	} else {
		self.resizeTo(635, 455);
	}
	checkOpertion();
...
function setprimaryPerson(){
    ...
    var def= document.getElementById("primaryPreDefine").value;
    var def_all = def.split(";");
    for(var i=0;i<def_all.length;i++){
		if(def_all[i]=="2"){
			primaryPreDefine += "流程起草人;";
		}
		if(def_all[i]=="3"){
			primaryPreDefine += "流程起草人上级;";
		}
		if(def_all[i]=="4"){
	...
	var nodeVariables = "";
	var variablesString = document.getElementById("variablesString").value;
	...
	var nodeArraryString = document.getElementById("nodeArraryString").value;
...
function setminorPerson(){
	var minorPreDefine="";
    var def= document.getElementById("minorPreDefine").value;
    var def_all = def.split(";");
    for(var i=0;i<def_all.length;i++){
		if(def_all[i]=="2"){
	...
	var nodeVariables = "";
	var variablesString = document.getElementById("minorVariablesString").value;
	var str=document.getElementById("nodeVariables").value;
	var variables=str.split(";");
	for(i=0;i<variables.length-1;i++){
	if(variablesString.indexOf(variables[i].split(",")[0])!=-1){
	...
	var node = "";
	var nodeArraryString = document.getElementById("minorNodeArraryString").value;
	var nodeStr=document.getElementById("manualNode").value;
	var nodes = nodeStr.split(";");
    for(i=0;i<nodes.length-1;i++){
        if(nodeArraryString.indexOf(nodes[i].split(",")[0])!=-1){
