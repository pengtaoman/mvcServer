/*
 * AUTHOR: YUHAO CREATION DATE:2008-02-27 WORKFILE:categoryManager.js
 * DESCRIPTION: Javascrpt file for categoryManager.jsp.It is build up Ext 1.1.1.
 * Copyright: ? 2008 东软 基础软件事业部版权所有
 */
var Action_GetCategory = WEB_APP_CONTENT + "/application.do?method=getCategory";
var Action_AddCategory = WEB_APP_CONTENT + "/application.do?method=addCategory";
var Action_DeleteCategory = WEB_APP_CONTENT
		+ "/application.do?method=deleteCategory";
var Action_ModifyCategory = WEB_APP_CONTENT
		+ "/application.do?method=modifyCategory";
	...
	return {
		...
		createDialog : function() {
			...
			if (!dialog) { // lazy initialize the dialog and only create it
				...
				submitBtn = dialog.addButton(submitText,
						dialogScope.dialogSubmit, this);
				closeBtn = dialog.addButton(closeText, dialogScope.dialogClose,
						this);
			}
		},
...
Ext.onReady(function() {
	...
	ctree.on('click', function() {
		...
		// alert(contextPath);
		window.open(contextPath + "/applicationresource.do?parentIdStore="
				+ parentIdStore + "&deleteId=all", "applicationResource");
	});
	...
	function saveModify(node) {
		...
		if ((node.attributes.type == oldNode.attributes.type)
				&& (node.attributes.text == oldNode.attributes.text)) {
			return;
		}
		var param = [];
		var parentIdStore;

		if (node.attributes.type == 'categorytype'
				|| node.parentNode.attributes.type == 'categorytype') {
			parentIdStore = '-1';
		} else {
	...
	function removeService(btn) {
		...
		if (btn == 'yes') {
			...
			if (node.attributes.type == 'category'
				...
				Ext.MessageBox.alert('Status', " 您要删除的分类“"+node.attributes.text+"”含有子分类，<br>请从子分类开始逐步删除。");
