...
{
  if (modifyAgentForm.assignee.value==modifyAgentForm.assigner.value){
     warning_info.innerHTML ="<%=MessageUtil.getString(
								"workflow.system.sysagent.check.checked",
								request.getSession())%>";
     return;
  }
  if (modifyAgentForm.startTime.value>modifyAgentForm.endTime.value){
     warning_info.innerHTML ="<%=MessageUtil.getString("workflow.check.time.checked",
								request.getSession())%>";
     return;
  }
  if (modifyAgentForm.assigner.value==""){
     warning_info.innerHTML ="<%=MessageUtil.getString(
								"workflow.system.sysagent.psn.checked", request
										.getSession())%>";
     return;  
  }
  if (modifyAgentForm.assignee.value==""){
     warning_info.innerHTML ="<%=MessageUtil.getString(
								"workflow.system.sysagent.psn.checked", request
										.getSession())%>";
     return;  
  }     
  modifyAgentForm.operation.value = action;
  document.modifyAgentForm.submit();
}
...
<uniflow:p_body onload="javascript:onload()" width="90%">
	<uniflow:m_form action="agentmodify.do">
		...
		<html:hidden property="id" value="<%=agentid%>" />
