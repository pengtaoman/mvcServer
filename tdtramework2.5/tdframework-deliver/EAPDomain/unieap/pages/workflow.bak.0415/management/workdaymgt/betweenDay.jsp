...
function button_onclick(action){
 if(freedayForm.startTime.value==""&&action=="OK"){
  ...
  //if(action=="OK"&&(!document.forms[0].startTime.onvalidate())) return;
  ...
  //if(action=="OK"&&(!document.forms[0].endTime.onvalidate())) return;
...
<uniflow:p_body onload="javascript:onload()" width="90%">
	...
	<uniflow:m_form action="between.do">

		<uniflow:tab>
			<uniflow:tabElement messageKey="workflow.schedule.bewteen"
				selected="true" action="javascript:button_onclick('betweenday')" />
		...
		<uniflow:p_content_wrapper>
			...
			<uniflow:p_content_table>
				...
				<uniflow:p_content_tr>
					<html:hidden property="worktimeType" value="1" />
					<!--<td class="main_list_td" valign="middle">
						组织单元
					</td>
					<td class="main_label_td" valign="middle" nowrap>
						<html:text property="orgunit" styleClass="input_text150"
