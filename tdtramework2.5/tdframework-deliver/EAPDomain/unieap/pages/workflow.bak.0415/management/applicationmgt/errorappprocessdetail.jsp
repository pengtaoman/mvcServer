...
<html>
		...
		<script type="text/javascript">
			...
			function query(){
				...
				var ddc=unieap.Action.requestData({
						url:unieap.WEB_APP_NAME+ '/LoadErrorApps.do?method=loadErrorApps&type=<%=request.getParameter("type")%>',
						sync:true,
    ...
    <body class="unieap" style="visibility:hidden;">
		<div id="grid" jsId="grid" dojoType="unieap.grid.Grid" width="100%" height="500px"
					...
					<header>
						<cell width="100px" label="实例名" name="instName" styles="text-align:center"></cell>
						<cell width="100px" label="事件名" name="event_id" styles="text-align:center"></cell>
					 ...
					 <toolbar paging="{parameters:{procInstID:'<%=request.getParameter("procInstID")%>','type':'<%=request.getParameter("type")%>'},url:'/LoadErrorApps.do?method=loadErrorApps'}">
