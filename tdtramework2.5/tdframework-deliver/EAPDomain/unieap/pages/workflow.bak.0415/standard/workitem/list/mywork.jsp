...
function complete_onclick(){
  ...
  if(needCommitRole=="true"){
    ...
    document.personWorkListForm.operation.value = "complete";
    document.personWorkListForm.action = "mywork.do";
    document.personWorkListForm.submit();
  }
}
...
function itemChanged()
{
  ...
  if(personWorkListForm.selectedItem){
    if(personWorkListForm.selectedItem.length)
    {
	for (i = 0; i < personWorkListForm.selectedItem.length; i++) {
	  rb = personWorkListForm.selectedItem[i];
	  if (rb.checked) {
	    checkedItem = rb;
	    isADHoc = personWorkListForm.ADHoc[i].value;
	    isReasign = personWorkListForm.reassignedWI[i].value;
	    curState = personWorkListForm.curState[i].value;
	    break;
	  }
	}
    }
    ...
    {
	checkedItem = personWorkListForm.selectedItem;
	isADHoc = personWorkListForm.ADHoc.value;
	isReasign = personWorkListForm.reassignedWI.value;
	curState = personWorkListForm.curState.value;
    }
...
function reload(){
  location.href = "mywork.do?state="+"<%=state%>";
}
function refresh(){
...
<uniflow:m_form action="mywork.do">
		...
		<table height="28" border="0" cellpadding="0" cellspacing="0"class="main_label_table">
           <tr><td height="25" valign="middle" nowrap class="main_label_td">
              <select onchange="javascript:changeto('allwork.do')" 
					...
					<option value="psn" selected><bean:message bundle="uniflow" key="workflow.workitem.list.person" /></option>
   ...
   <uniflow:m_table style="main_list">
	 ...
	 <uniflow:m_order_th name="name"    value="workflow.workitem.name"
	    ...
	    orderKey='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("personWorkListForm")).getOrderBy()%>' 
	    ascending='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("personWorkListForm")).isAscending()%>'/>
   ...
   <uniflow:order_th value="workflow.workitem.procinstname" /> 
	 <logic:iterate id="workitem" name="personWorkListForm" property="list" type="com.neusoft.uniflow.api.handler.NWWorkItem" indexId="index">
	...
	<td valign="middle" class="main_list_td"><%=CommonInfoManager.getProcInstName(workitem.getWorkItemID())%>&nbsp;</td>
		...
		<tr><td>
			<uniflow:pageCtr
				curPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("personWorkListForm")).getCurrentPage()%>'
				maxPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("personWorkListForm")).getPagesCount()%>'
				total='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("personWorkListForm")).getTotal()%>' />
		</td><td align="right" >
