...
<%@ page import="com.neusoft.uniflow.web.common.list.OpenListForm"%>
...
function complete_onclick()
{
  ...
  if(needCommitRole=="true"){
    ...
    document.roleWorkListForm.operation.value = "complete";
    document.roleWorkListForm.action = "rolework.do";
    document.roleWorkListForm.submit();
  }
}
...
function take_onclick()
{
  document.roleWorkListForm.operation.value = "take";
  document.roleWorkListForm.action = "rolework.do";
  document.roleWorkListForm.submit();
}
...
function itemChanged()
{
  ...
  if(roleWorkListForm.selectedItem){
    if(roleWorkListForm.selectedItem.length)//

	for (i = 0; roleWorkListForm.selectedItem.length>i; i++) {
	  rb = roleWorkListForm.selectedItem[i];
	  if (rb.checked) {
	    checkedItem = rb;
	    isADHoc = roleWorkListForm.ADHoc[i].value;
	    isReasign = roleWorkListForm.reassignedWI[i].value;
	    curState = roleWorkListForm.curState[i].value;
	    break;
	  }
	}
    else
    {
	checkedItem = roleWorkListForm.selectedItem;
	isADHoc = roleWorkListForm.ADHoc.value;
	isReasign = roleWorkListForm.reassignedWI.value;
	curState = roleWorkListForm.curState.value;
    }
...
function refresh()
{
  location.href = "rolework.do?state="+"<%=state%>&op=<%=operation%>";
}
...
<uniflow:m_form action="rolework.do">
		...
		<table height="28" border="0" cellpadding="0" cellspacing="0"class="main_label_table">
           <tr><td height="25" valign="middle" nowrap class="main_label_td">
              <select onchange="javascript:changeto('allwork.do')" 
					...
					<option value="role" selected><bean:message bundle="uniflow" key="workflow.workitem.list.role" /></option>
   ...
   <uniflow:m_table style="main_list">
	 ...
	 <uniflow:m_order_th name="name"   value="workflow.workitem.name"
	    ...
	    orderKey='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("roleWorkListForm")).getOrderBy()%>' 
	    ascending='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("roleWorkListForm")).isAscending()%>'/>
   ...
   <uniflow:order_th value="workflow.workitem.procinstname" /> 
	 <logic:iterate id="workitem" name="roleWorkListForm" property="list" type="com.neusoft.uniflow.api.handler.NWWorkItem" indexId="index">
	...
	<td valign="middle" class="main_list_td"><%=CommonInfoManager.getProcInstName(workitem.getWorkItemID())%>&nbsp;</td>
		...
		<tr><td>
			<uniflow:pageCtr
				curPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("roleWorkListForm")).getCurrentPage()%>'
				maxPage='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("roleWorkListForm")).getPagesCount()%>'
				total='<%=((com.neusoft.uniflow.web.common.list.OpenListForm) request.getAttribute("roleWorkListForm")).getTotal()%>' />
		</td><td align="right" >
		...
		<%}else if(operation.equals("all")){ %> 
            ...
            <uniflow:button id="take" name="button.take"action="javascript:take_onclick()"  />
        <%} %>
