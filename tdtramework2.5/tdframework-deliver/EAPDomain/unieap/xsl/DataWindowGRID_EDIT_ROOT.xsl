...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
+
+	文件说明： 数据窗口解析模板集，仅适用于类型为GRID_EDIT的数据窗口。
...
<xsl:template name="template-grideditroot-buttons">
	    <td align="right">
			...
			<xsl:for-each select="/dataWindow/buttons/button[@type=$DW_BUTTON_PAGE_TYPE]">
				<input type="button" class="NEUDwButton" value="首页" onclick="dsSessionMgr('loadFirstPage','{$DW_FORM_NAME}','{/dataWindow/@name}')"/>
				<input type="button" class="NEUDwButton" value="前页" onclick="dsSessionMgr('loadPreviousPage','{$DW_FORM_NAME}','{/dataWindow/@name}')"/>
				<input type="button" class="NEUDwButton" value="后页" onclick="dsSessionMgr('loadNextPage','{$DW_FORM_NAME}','{/dataWindow/@name}')"/>
				<input type="button" class="NEUDwButton" value="末页" onclick="dsSessionMgr('loadLastPage','{$DW_FORM_NAME}','{/dataWindow/@name}')"/>
				<input type="button" class="NEUDwButton" value="刷新" onclick="dsSessionMgr('reloadCurrentPage','{$DW_FORM_NAME}','{/dataWindow/@name}')"/>
			</xsl:for-each>
	    </td>
