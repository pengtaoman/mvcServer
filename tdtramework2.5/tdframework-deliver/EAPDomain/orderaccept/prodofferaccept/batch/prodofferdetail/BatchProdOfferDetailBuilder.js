defineModule("orderaccept.prodofferaccept.batch.prodofferdetail.BatchProdOfferDetailBuilder", [
                "../../util",
                "../../../base/Controller",
                "../../widget/attrcard/ProductAttrCardWidget",
                "../../widget/attrcard/ProductOfferAttrCardWidget",
                "./BatchProdOfferAssureBuilder",
                "../../../custom/TooltipDialog",  
                "./BatchFavour",   
                "orderaccept.common.js.ConstantsPool",  
				"orderaccept.common.dialog.MessageBox",
				"orderaccept.prodofferaccept.builder.prodofferdetail.ProdOfferDetailBuilder",
                ...
                ProductOfferAttrCardWidget, ProdOfferAssureBuilder, TooltipDialog, Favour, ConstantsPool,messageBox,prodOfferDetailBuilder) {
	        /**
			 * 定义销售品详情整体构造器
			 * 
			 * @class
			 * @extends
			 */
	        dojo.declare("orderaccept.prodofferaccept.batch.prodofferdetail.BatchProdOfferDetailBuilder", [prodOfferDetailBuilder], {
				 ...
				 * 
				 * @param {loader}
				 * 入参provider是个对象，对象中必须包含controller对象，拼个空对象就可以
				 */
...
//				        })[0],
			        ...
			        var prodOfferDetailWidget = new TooltipDialog({
				                id : 'prodOfferDetailDialog-'+prodOfferInfo.prodOfferId
			                });    
					var rowIndex = prodOfferInfo.prodOfferId;
					rowData.rowIndex  = prodOfferInfo.prodOfferId;
			        this.prodOfferDetailWidgetList["" + rowIndex] = prodOfferDetailWidget;
			        
			        /****************可选包上的销售品详情信息初始化 start************/
			        window.attrUsedPage = "batchPage";
			        ...
			        var commitButton = "<div style='text-align:center;vertical-align:middle'><a href='javascript:void(0);' dojoAttachEvent='onclick:elementclick' \ rowIndex="+rowIndex+"\
			        		dojoAttachTopic='/batchProdOfferDetailBtn'\ style='text-align:center;vertical-align:middle;'>关闭</a></div>";	
...
//			        
//			        if(!prodOfferInst || (!prodOfferInst.prodOfferAssureInfoList || prodOfferInst.prodOfferAssureInfoList.length == 0)){
...
//					}
			        prodOfferDetailWidget.handlePageData = dojo.hitch(prodOfferDetailWidget,
				        function() {
				        	...
				        	if(!!prodOfferDetailWidget.prodOfferAttrCard){
				        		...
				        		if(prodOfferAttr){
			           	    		dojo.mixin(resultData,{offerAttrPageInfoList:prodOfferAttr});
				        		}else{
				        			return false;
				        		}
				        		//设置定价参数信息
				        		...
				        		if(pricingParamCardInfo){
				        			dojo.mixin(resultData,{offerAttrPageInfoList:pricingParamCardInfo});
				        		}
				        	}
				        	//设置亲情信息
			           	    if(!!prodOfferDetailWidget.favourBuilder){
			           	  		dojo.mixin(resultData,{relaBusPageInfoList:prodOfferDetailWidget.favourBuilder.getAllFavourData()});
			           	    }
				        	//设置产品信息
			           	    var relaProdPageInfoList = builder.getProductData(rowIndex);
			           	    if(relaProdPageInfoList){
			           	    	dojo.mixin(resultData,{relaProdPageInfoList:relaProdPageInfoList});
			           	    }else{
			           	    	return false;
			           	    }
			           	    ...
			           	    dojo.mixin(resultData,{prodOfferAssurePageInfoList:prodOfferDetailWidget.assurePageData});
		                	return resultData;
		                }
	                );
		        ...
		        getData : function(){
		        	var builder = this;
		        	var dataMap = {};
		        	var prodOfferDetailWidgetList = builder.prodOfferDetailWidgetList;
		        	for(var p in prodOfferDetailWidgetList){
		        		if (!prodOfferDetailWidgetList.hasOwnProperty(p)) {
					        continue;
				        }
		        		var prodOfferDetailWidget = prodOfferDetailWidgetList[p];

						var offerDivs = dojo.query("div[id=relaOfferList]");
						if(!!offerDivs && !!offerDivs[0]){							
							
							var prodOfferDivs = dojo.query("[value="+p+"]",offerDivs[0]);
							if(!!prodOfferDetailWidget && !!prodOfferDivs&& !!prodOfferDivs[0]&& !prodOfferDivs[0].checked==true){
					        	continue;
							}
						}
		        		var data = prodOfferDetailWidget.handlePageData();
		        		if(!data){
		        			return false;
		        		}
		        		dataMap[p]=data;
		        ...
		        initProductAttr : function(prodOfferDetailWidget,rowData, tabContainer){
			        ...
			        //过滤非接入类
			        var prodList = this.filterRelaProdListForSpec(rowData);
			        //prodList = rowData.subProdOfferInfo.offerProdRelaList;
		        ...
		        filterRelaProdListForSpec : function(rowData){
										...
										try{										 	
					                		...
					                		var tipPane = util.DomHelper.getParentWidget(cardNode,
							              	...
							              	if(toolTip.id.indexOf("mainProdOfferDetailDialog") >= 0 
							              		...
							              		detailFlag = dojo.byId("prodOfferLink-"+toolTip.rowIndex);
							              	}
