defineModule("orderaccept.prodofferaccept.builder.prodofferdetail.Favour", ["../../util",
			 ...
			 * @extends
			 */
	        dojo.declare("orderaccept.prodofferaccept.builder.prodofferdetail.Favour.FavourBuilder", [], { 
			...
			//				favourItemList.push(this.hasFavourItemList[i]);
				...
				createFavourItem : function(favourNumberObj){
					...
					var favourItem = new orderaccept.prodofferaccept.builder.prodofferdetail.Favour.FavourItem(args);
			//		var favourItemData = favourItem.createItemData();
				...
				createFavourNumberHtmlByData : function(hasFavourItemData){
					...
					if(builder.FavourKindConst.COMMON_FAVOUR == builder.favourType){
						...
						if(!!hasFavourItemData.unitId && !builder.unitId){
							builder.unitId = hasFavourItemData.unitId;
						}
					...
					if(!hasFavourItemData.operKind){
						hasFavourItemData.operKind = 2;
					}
					favourReturnTempItem.operKind = hasFavourItemData.operKind;
			    ...
			    */
				...
				doCheck : function(favourNumber,subGroupType){
					favourNumber = BusCard.util.trim(favourNumber);
					//判断是否输入自身
					...
					//if(this.FavourKindConst.SELF_FAVOUR != this.favourType && favourNumber.length != 11){ //自惠可以添加固网号码
					//	messageBox.alert({
					//		busiCode : "08410111",
					//		infoList : [ "\u4eb2\u60c5\u53f7\u7801\u5e94\u4e3a11\u4e3a\u624b\u673a\u53f7\u7801" ]
					// 	}, $('familyNumber'));
					//	return false;
					//}
					//判断是否重复输入
		 ...
		 * 销售品亲情号码项
		 */
        dojo.declare("orderaccept.prodofferaccept.builder.prodofferdetail.Favour.FavourItem", [], { 
				Me : this,
	        	constructor : function(args){		
					...
					this.beginDate = !!this.favourNumberObj.beginDate?this.favourNumberObj.beginDate:$ac$.requestParam.today;
				...
				createItemData : function(){
					...
					if(!objServiceKind || objServiceKind/1 < 0){
						objServiceKind = 0;
					}
					...
					var relaBusData = {
						"cityCode" : "",
						"userId" : !!this.builder.userId?this.builder.userId:"0",
						"serviceId" : "",
						"serviceKind" : "0",
				...
				handleFavourNumber : function(){
					...
					if(checkBoxObj.checked == true){
				    	...
				    	//亲情号码的检测				        
						//检测输入的亲情号码的最大的个数
						var favourRelaLimitXML = dojo.toJson(favourItem.builder.getRelaLimitData());//亲情限制json串
						var relaNumberXML = dojo.toJson(favourItem.builder.getAllFavourData());//已存在的亲情号码集合 
						var parameters = "&favourNumber="+favourItem.favourNumber;
						parameters += "&favourRelaLimitXML="+favourRelaLimitXML;
						parameters += "&relaNumberXML="+relaNumberXML;
						parameters += "&favourType="+favourItem.builder.favourType;
						parameters += "&unitId="+favourItem.builder.unitId;
						parameters += "&dojoType=1";
				        var resultJsonObj = util.ServiceFactory
			                .getService("url:prodOfferSaleAjaxAction.do?method=doCheckRelaNumber" + parameters);
						if(!resultJsonObj){					
							messageBox.alert({
						 	...
						 	$("familyNumberCh-"+favourItem.getFavourNumber()).checked = false;
							return false;
						}
						if(resultJsonObj.code/1 < 0 && resultJsonObj.message){
							 messageBox.alert({
					...
					var hasFavourList = this.builder.hasFavourItemList;
					if(!!hasFavourList && hasFavourList.length > 0 && this.favourType == this.builder.FavourKindConst.OCS_FAVOUR){
						var addNumDom = dojo.query("[id=addNumber]",this.builder.prodOfferDetailWidget.domNode)[0];
						if(!!addNumDom){
							dojo.style(addNumDom,"display","none");
						}
					}
				},
				/**
