...
<%
	...
	String governmentCustCount="0";//河北需求，辽宁没有暂时屏蔽掉   NullProcessUtil.nvlToString(request.getAttribute("governmentCustCount"), "0");	//查询客户是否是存政企客户。存政企客户只展现下面的用户，不能够办理业务。

	String custId= NullProcessUtil.nvlToString(request
			.getAttribute("custId"), "-1");	//客户Id
	String showNumberInfo = NullProcessUtil.nvlToString(request
			.getAttribute("showNumberInfo"), "");	//客户级查询最大显示数提示信息
	String oldCardMessage = NullProcessUtil.nvlToString(request
					.getAttribute("oldCardMessage"), "");	//旧卡提示换新卡信息		
%>
<html>
	<head>
	...
	<body onload="checkedOrder()" class="unieap mainBody">
		...
		<input type="hidden" id="custId" name="custId"
			value="<%=custId%>" />
		<input type="hidden" id="showNumberInfo" name="showNumberInfo"
			value="<%=showNumberInfo%>" />
		<input type="hidden" id="oldCardMessage" name="oldCardMessage"
			value="<%=oldCardMessage%>" />		
					...
					<table width="100%" border="1" cellpadding="0" cellspacing="0"
						...
						<tr class="table_top" onmouseover='setColor(this)'
							...
							<%
								if (changeButton1) {
								%>
								<button id="A-<%=prodOfferInstId %>-<%=prodOfferId %>" class="titleButton" style="text-align: left;width: 65px" 
									onclick="showOrderDetail(<%=prodOfferInstId%>,<%=prodOfferId%>,<%=ifBundle%>,<%=productId%>,this);">销售品变更</button>
								<!-- <a href="javascript:void(0);" id="A-<%=prodOfferInstId %>-<%=prodOfferId %>"
									onclick="return showOrderDetail(<%=prodOfferInstId %>,<%=prodOfferId %>,this);">变更</a> -->
								<%
								 }if (1 == pOfferInfoVOList.get(i).getBindType()) {
								%>
								<!--  <a href="#"
									onclick="return prodSplit(<%=prodOfferInstId %>,<%=prodOfferId %>);">拆分
									|
									<button class="titleButton" 
										onclick="prodInterchange(<%=prodOfferInstId %>,<%=prodOfferId %>);">互换</button>-->
									<!-- <a href="javascript:void(0);"
									onclick="return prodInterchange(<%=prodOfferInstId %>,<%=prodOfferId %>);">互换 -->
								 		<%
									if (pOfferInfoVOList.get(i).isIfExistMounted()) {
								%> <!--|<a href="#"
										onclick="return prodAdded(<%=prodOfferInstId %>,<%=prodOfferId %>);">加装-->
											<%
									}
								}
								%>
							...
							<td colspan="12">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<tr class="table_content" onmouseover='setColor(this)'
										...
										<td width="17%">
										<% 
											if(changeButton2 && ConstantsPool.get(ProductFuncTypeConst.ACCESS).equals(relaProdInfoVOList.get(j).getProdFuncType())){
										%>
											<button id="A-<%=productInstId %>-<%=serviceId%>" class="titleButton" style="text-align: left;width: 55px"
												onclick="showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">产品变更</button>
											<!-- <a id="A-<%=productInstId %>-<%=serviceId%>" href="javascript:void(0);"
												onclick="return showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">变更</a> -->
										<%
											}
										%>
										</td>
										...
										<td colspan="12">
											<table width="100%" border="0" cellspacing="0"
												...
												<tr class="table_content" onmouseover='setColor(this)'
													...
													<td width="17%">
													<% 
													if(changeButton3 && ConstantsPool.get(ProductFuncTypeConst.ACCESS).equals(relaProdInfoVOListTwo.get(m).getProdFuncType())){	
													%>
														<button id="A-<%=productInstId %>-<%=serviceId%>" class="titleButton" style="text-align: left;width: 65px"
															onclick="showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">产品变更</button>
														<!-- <a id="A-<%=productInstId %>-<%=serviceId%>" href="javascript:void(0);"
															onclick="return showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">变更</a> -->
													<%
													...
													<td colspan="12">
														<table width="100%" border="0" cellspacing="0"
															...
															<tr class="table_content"
																...
																<td width="17%">
																<% 
																	if(changeButton4 && ConstantsPool.get(ProductFuncTypeConst.ACCESS).equals(relaProdInfoVOListThree.get(n).getProdFuncType())){
																%>
																	<button id="A-<%=productInstId %>-<%=serviceId%>" class="titleButton" style="text-align: left;width: 65px"
																		onclick="showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">产品变更</button>
																	<!-- <a id="A-<%=productInstId %>-<%=serviceId%>" href="javascript:void(0);"
							...
							<td colspan="12">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<tr class="table_content"  style="background-color: #E7F1D8">
										...
										<td width="17%">
										<% if(changeButton1){ %>
										<!-- delete by zhuguojun 20120607 晓凤确认销售品下的销售品不显示变更按钮。
											<button id="B-<%=prodOfferInstId %>-<%=prodOfferId %>" class="titleButton" style="text-align: left;width: 65px"
												onclick="showOrderDetail(<%=prodOfferInstId %>,<%=prodOfferId %>,this);">销售品变更</button>-->
											<!-- <a href="javascript:void(0);" id="B-<%=prodOfferInstId %>-<%=prodOfferId %>"
												onclick="return showOrderDetail(<%=prodOfferInstId %>,<%=prodOfferId %>,this);">变更</a> -->
											<%
											if (1 == pOfferInfoVOList.get(i).getBindType()) {
											%>
										...
										<td colspan="12">
											<table width="100%" border="0" cellspacing="0"
												...
												<tr class="table_content" onmouseover='setColor(this)'
													...
													<td width="17%">
													<% //changeButton6 变为 changeButton2：只要是接入类，都可以变更，权限相同。
													if(changeButton2 && ConstantsPool.get(ProductFuncTypeConst.ACCESS).equals(relaOfferInfoList.get(j).getProdFuncType())){
													%>
													<button id="A-<%=productInstId %>-<%=serviceId%>" class="titleButton" style="text-align: left;width: 65px"
														onclick="showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">产品变更</button>
													<!-- <a id="A-<%=productInstId %>-<%=serviceId%>" href="javascript:void(0);" onclick="return showProdOrderDetail('<%=prodOfferInstId %>','<%=prodOfferId %>','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">变更</a> 	 -->
													<%
														}
													%>													
													</td>
						...
						<tr class="table_content" onmouseover='setColor(this)'
							...
							<td width="9%" id="serviceId-<%=serviceId%>">
								<%=serviceId%>
							</td>
							<td width="10%">
								<%=prodStatus%>
							...
							<td width="15%">
								<button id="B-<%=productInstId %>-<%=serviceId%>" class="titleButton" 
									onclick="showProdOrderDetail('','','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">产品变更</button>
								<a id="B-<%=productInstId %>-<%=serviceId%>" href="javascript:void(0);"
									onclick="return showProdOrderDetail('','','<%=prodId %>','<%=productInstId %>','<%=serviceId%>','<%=cityCode %>',this);">变更</a> 
							</td>
						</tr> -->
							<%
							//}
